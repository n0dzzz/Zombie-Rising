...
local function CreateScoreboard(ToggleScoreboard)
    if ToggleScoreboard then
        ...
        PlayerScrollPanel:SetPos(2,ScoreboardBase:GetTall() * 0.05)
        PlayerScrollPanel:SetSize(ScoreboardBase:GetWide() - 4,ScoreboardBase:GetTall() * 0.95)
        for k,v in pairs(player.GetAll()) do
            ...
            PlayerPanel.Paint = function(self,w,h)
                if IsValid(v) then
                    ...
                    draw.SimpleText(PlayerMoney, "TargetID", w * 0.384 , h /2 , Color(0,0,0,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
